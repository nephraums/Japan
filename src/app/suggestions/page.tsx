  ...
  return (
    <AppShell profile={data.profile} demo={data.demo}>
      <div className="mx-auto max-w-4xl px-4 py-6 sm:px-6 sm:py-9">
        ...
        {readOnly && <p className="mt-5 rounded-xl bg-[#efe5d6] p-3 text-sm font-semibold text-[#655e56]">Guest view is read-only. You can browse ideas and votes, but only the family can add ideas or vote.</p>}

        {!readOnly && <details className="group mt-6 rounded-[1.6rem] border border-[#ded3c3] bg-white/75 shadow-[0_12px_35px_rgba(66,51,38,.06)]">
            ...
            return (
              <article key={suggestion.id} className="rounded-[1.6rem] border border-white/80 bg-white/80 p-5 shadow-[0_12px_35px_rgba(66,51,38,.07)]">
                ...
                <div className="mt-4 flex flex-wrap items-center gap-2">
                  ...
                  {readOnly && <span className="inline-flex min-h-11 items-center gap-2 rounded-xl border border-[#ded3c3] bg-white px-4 text-xs font-bold text-[#716a62]">Votes: Yes {suggestion.votes_for} · No {suggestion.votes_against}</span>}
